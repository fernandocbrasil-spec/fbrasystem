...
export default function FinanceiroAprovacaoDetalhePage() {
    ...
    if (!item) {
        return (
            <div className="py-12 text-center">
                <p className="text-sm text-pf-grey">Titulo nao encontrado.</p>
                <Link href="/financeiro/aprovacoes" className="text-xs text-pf-blue hover:underline mt-2 inline-block">
    ...
    const isPending = status === "desconto_solicitado" || status === "baixa_solicitada";
    const actionLabel = item.requestedAction === "desconto" ? "Desconto" : "Baixa";
    ...
    return (
        <div className="space-y-6">
            <PageHeader
                title={`Aprovacao de ${actionLabel}`}
                subtitle={`${item.cliente} — ${item.descricao}`}
                actions={
                    <Link
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="rounded-lg border border-pf-grey/10 bg-white p-6 shadow-sm">
                        <div className="flex items-center gap-4 mb-6 pb-6 border-b border-pf-grey/10">
                            ...
                            <div>
                                <h2 className="font-sans text-2xl font-bold text-pf-black">{item.cliente}</h2>
                                <p className="font-sans text-sm text-pf-grey">{item.descricao}</p>
                            </div>
                        ...
                        <div className="grid grid-cols-2 gap-y-6 text-sm">
                            <div>
                                <p className="text-xs font-bold uppercase text-pf-grey mb-1">Tipo de Solicitacao</p>
                                <p className="text-pf-black font-bold">{actionLabel}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                <p className="text-xs font-bold uppercase text-pf-grey mb-1">Valor Original</p>
                                <p className="text-pf-black font-sans text-lg font-bold">{item.valor}</p>
                            </div>
                            {item.requestedValue && (
                                <div>
                                    <p className="text-xs font-bold uppercase text-pf-grey mb-1">Valor Solicitado</p>
                                    <p className="text-pf-blue font-bold text-lg">{item.requestedValue}</p>
                                </div>
                            )}
                            <div>
                                <p className="text-xs font-bold uppercase text-pf-grey mb-1">Banco</p>
                                <p className="text-pf-black">{item.banco}</p>
                            </div>
                            {item.requestedBy && (
                                <div>
                                    <p className="text-xs font-bold uppercase text-pf-grey mb-1 flex items-center gap-1.5">
                                    ...
                                    <p className="text-pf-black">{item.requestedBy}</p>
                                </div>
                            )}
                            {item.requestedReason && (
                                <div className="col-span-2">
                                    ...
                                    <p className="text-pf-black bg-pf-grey/5 p-3 rounded-md">{item.requestedReason}</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    {isPending ? (
                        <div className="rounded-lg border border-pf-blue/20 bg-pf-blue/5 p-6 shadow-sm">
                            ...
                            <p className="text-xs text-pf-grey mb-6">
                                {item.requestedAction === "desconto"
                                    ? "Ao aprovar, o desconto sera aplicado ao titulo e o valor ajustado."
                                    : "Ao aprovar, o titulo sera baixado como incobravel."
                                }
                            </p>
                        ...
                        <div className="rounded-lg border border-pf-grey/10 bg-white p-6 shadow-sm">
                            ...
                            <div className="flex items-center gap-3">
                                <ApprovalBadge status={status} />
                                {status === "aprovado" && (
                                    <span className="text-xs text-pf-grey">Solicitacao aprovada com sucesso.</span>
                                )}
                                {status === "rejeitado" && (
                                    <span className="text-xs text-red-500">Solicitacao rejeitada.</span>
                                )}
                            </div>
                        </div>
                    )}
